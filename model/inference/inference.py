...
def masktopolygon(im_bg_dir, im_gray_dir, savepath):
    ...
    for name, im_bg_path in tqdm(files.items(), desc='Draw polygon'):
        ...
        if osp.exists(im_gray_path):
            ...
            cv2.drawContours(imbgr, contours, -1, (255, 0, 0), 2)
...
def main():
    ...
    if args.model == 'unet':
        ...
        config_file = '/home/jovyan/model/jbnu/my_model/segformer/segformer_mit-b2_512x512_160k_ade20k.py'
        save_mask_root = f'{args.output}/segformer_mask/'
    else:
        config_file, save_mask_root = None, ''
        assert ValueError('The input parameter --model must be unet or deeplabv3 or segformer, but you use %s'%args.model)
